...
export const UserList = ({ users ,getUsersLoading}) => {
  const [searchTerm, setSearchTerm] = useState("");
  const [statusFilter, setStatusFilter] = useState("all");
  const [changeUserRole, { error, isLoading }] = useChangeUserRoleMutation();
  const [toggleUserBlock, { error: blockError,isLoading : l }] = useToggleUserBlockMutation();
  ...
  const filteredUsers = users.filter((user) => {
    ...
    if (l || isLoading || getUsersLoading ) {
    return (
  ...
  <div className="bg-white rounded-2xl shadow-xl border border-gray-100 w-full max-w-full">
    {/* Filters */}
