...
const handleRemoveImage = (index: number, imgIndex: number) => {
  ...
  // If no images left, clear currentUpload for this variant
  if (updated.length === 0) {
    setCurrentUpload((prev) => {
  ...
  for (let i = 0; i < data.variants.length; i++) {
    if (data.variants[i].images.length === 0) {
      alert(`Variant ${i + 1} must have at least one image`);
      return;
    }
  }
    try {
      ...
      console.log(err);
      toast.error(err?.data?.message || "Somwthing went wrong")
    }
  };
  ...
  return (
    <form
      className="w-full flex flex-col gap-10 bg-white rounded-2xl p-6"
    ...
    maxLength: { value: 50, message: "Maximum 50 characters" },
              ...
              className="py-[10px] px-4 h-12 w-full border border-[#232321] rounded-xl"
            />
    ...
    maxLength: { value: 300, message: "Maximum 300 characters" },
              ...
              className="min-h-[180px] h-full py-[10px] px-4 w-full border border-[#232321] rounded-xl"
            />
...
<div className="flex flex-col gap-2 flex-1">
  ...
  <input
    ...
    disabled={selectedType !== "hybrid" && selectedType !== "loyalty_points"}
                  ...
                  render={({ field }) => (
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (!e.target.checked) setValue("discountPercentage", 0);
                      }}
                      className="w-5 h-5"
                    />
                  )}
      ...
      >
        Remove Variant
             ...
             <div className="flex flex-col gap-2 flex-1"> 
                ...
                <input type="text" placeholder="Enter Color" {...register(`variants.${index}.color`, { required: true })} className="py-[10px] px-4 h-12 w-full border border-[#232321] rounded-xl" /> 
                {errors.variants?.[index]?.color && (
  <p className="text-red-500 text-sm">{errors.variants[index].color?.message}</p>
)}
              </div> 
              {/* Sizes */} 
              <div className="flex flex-col gap-2 flex-1"> 
                <label className="text-[#232321] font-semibold text-xl">Sizes (comma separated)</label> 
                <input type="text" placeholder="Enter sizes e.g. S,M,L" {...register(`variants.${index}.sizes`,{
                  required:true
                })} onBlur={(e) => { const sizes = e.target.value.split(",").map((s) => s.trim()); setValue(`variants.${index}.sizes`, sizes); }} className="py-[10px] px-4 h-12 w-full border border-[#232321] rounded-xl" /> 
                {errors.variants?.[index]?.sizes && (
  <p className="text-red-500 text-sm">{errors.variants[index].sizes?.message}</p>
)}
              </div>
              ...
              {currentUpload[index]?.length > 0 && (
                <div className="flex gap-4 flex-wrap mb-4 relative">
                  {currentUpload[index].map((url, i) => (
                    <div
                      key={i}
                      className="relative w-full h-80 rounded-xl overflow-hidden bg-[#00000033] flex items-center justify-center"
                    >
              ...
              <div className="relative w-[457px] h-[164px] rounded-xl border border-dashed border-[#232321] p-4 flex flex-col gap-4 justify-center">
                ...
                />
                <p className="text-center text-[#70706E] font-semibold">
                  ...
                  <li key={i} className="w-full flex gap-4 p-4 rounded-xl bg-[#FAFAFA] relative">
                    ...
                    <div className="flex flex-col justify-center gap-2 w-full">
                      ...
                      <div className="h-1 rounded-xl bg-[#4A69E2] w-full">
                        <div
                          className="bg-[#003F62] h-full rounded-xl"
                          style={{ width: `${uploadProgress[index] || 100}%` }}
                        />
                      </div>
                    </div>
                    {/* Remove Icon */}
          ...
          <Button
            type="button"
            onClick={() => append({ color: "", images: [], sizes: [] })}
            className="bg-[#003F62] h-12 rounded-xl mt-4"
          >
            Add Variant
          </Button>
        </div>
      ...
      <div className="flex gap-4 mt-6 max-w-[300px]" >
        <Button type="submit" className="bg-[#003F62] text-white h-12 w-full">
          Add
        </Button>
        <Button type="button" className="border border-[#232321] h-12 w-full">
          Cancel
        </Button>
      </div>
    </form>
